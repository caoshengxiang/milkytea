...
function swipeDirection (x1, x2, y1, y2) {
  ...
  Math.abs(y1 - y2) ? (x1 - x2 > 0 ? 'Left' : 'Right') : (y1 - y2 > 0
    ? 'Up'
    : 'Down')
}
...
Component({
  ...
  methods: {
    ...
    handlerTouchmove (event) {
      ...
      const touches = event.touches ? event.touches[0] : {}
      if (touches) {
        const direction = swipeDirection(start.pageX, touches.pageX,
          start.pageY, touches.pageY)
        if (direction === 'Left') {
