...
Component({
  ...
  methods: {
    navigateTo () {
      ...
      if (!this.data.isLink || !url || url === 'true' || url ===
        'false') return
      ...
      if (['navigateTo', 'redirectTo', 'switchTab', 'reLaunch'].indexOf(
        this.data.linkType) === -1) {
        warn('linkType 属性可选值为 navigateTo，redirectTo，switchTab，reLaunch',
          this.data.linkType)
        return
      }
